...
uint8_t string_animation_get_current_max_char(StringAnimation* string_animation) {
    return string_animation->current_max_char;
}
...
void string_animation_timer_process(StringAnimation* string_animation) {
    if(string_animation->str == NULL) return;
    if(LL_TIM_GetCounter(TIM4) > string_animation->timer_compare_value) {
